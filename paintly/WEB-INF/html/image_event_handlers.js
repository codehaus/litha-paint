...
function highlightImageFocus(evt){
   if(focusedImage && focusedImage.length>5){
	   ...
	   for(var i=0;i<9;i++){
	     if(mobilePoint[i] && x-canvas_x>=mobilePoint[i][0] && x-canvas_x<=mobilePoint[i][0]+mobilePoint[i][2] && y-canvas_y>=mobilePoint[i][1] && y-canvas_y<=mobilePoint[i][1]+mobilePoint[i][3]){          
		   highlight = true;
