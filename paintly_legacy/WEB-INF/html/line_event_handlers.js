...
function moveLine(evt){
	...
	if ((evt?(evt.button==65535 || evt.button == 0):event.button==1) && dragapproved == true){	
		...
		jg.setColor("#00FF00");
	    jg.setStroke(1);    
		jg.drawLine(x-canvas_x, y-canvas_y, x1-canvas_x, y1-canvas_y);
		jg.paint();
		left = x;
		top = y;
		width = x1-x;
...
function downPolylineImp(tx,ty,button){
	...
	if(button==2){//right button pressed
	  ...
	  if(tool_selected==5){
		  ...
		  jg.drawLine(polylineSteps[polylineSteps.length-1][0]-canvas_x, polylineSteps[polylineSteps.length-1][1]-canvas_y, polylineSteps[0][0]-canvas_x, polylineSteps[0][1]-canvas_y);
	  }
...
function movePolyline(evt){
	if(++counter%2 != 0 && polylineSteps.length>0){
		...
		jg.drawLine(polylineSteps[curStep][0]-canvas_x, polylineSteps[curStep][1]-canvas_y, x1-canvas_x, y1-canvas_y);
		if(tool_selected==5){jg.drawLine(polylineSteps[0][0]-canvas_x, polylineSteps[0][1]-canvas_y, x1-canvas_x, y1-canvas_y);}
		jg.paint();
		left = x;
		top = y;
		width = x1-x;
