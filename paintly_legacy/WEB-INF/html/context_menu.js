  ...
  this.show = function(x,y){
    ...
    if(this.menuItems.length > 0){
		...
		for(var i=0;i<this.menuItems.length;i++){
			...
			innerHTMLValue += '<div class="icon" onmouseover="highlightMenuItem(this)" width="96px" style="background-color:white;"  onmousedown="'+this.menuItems[i].func+'()">'
						   +  '<table><tr class="systemGreenFont"><td width="35" class="cursorHand"><img id="'+this.id+this.menuItems[i].num+'Img" src="' + this.menuItems[i].img.src + '"></td>'
						   +  '<td class="cursorHand" id="'+this.id+this.menuItems[i].num+'">' + this.menuItems[i].cap + '</td></tr></table></div>';
